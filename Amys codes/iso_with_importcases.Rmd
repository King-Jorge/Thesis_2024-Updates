...
### the isolation-only strategy (with importation) in terms of Hansen & Day
### in terms of the public health terminology that I defined
#### elimination, suppression, and circuit breaker. NB: Here elimination is not possible
...
##### SUPPRESSION/ELIMINATION (same parameters except a lower u1max)
u1max <- 0.7 
...
g.supp = ggplot(out.supp, aes(x = time, y = I1)) +
  ...
  geom_line(aes(y=iso))+ggtitle("Suppression")+ylab("Community prevalence") + xlim(0,T)+theme_classic()
g.supp



# Create a data frame for ggplot (Bar plots)
values <- c(out.supp$j[nrow(out.supp)], out.supp$C1[nrow(out.supp)])
...
# Create a data frame for ggplot (Bar plots)
values <- c(out.circ$j[nrow(out.circ)], out.circ$C1[nrow(out.circ)])
labels <- c("Cummulative new cases", "Resources used")
...
tstart1<-5
toff1<-1000
out4 <- ode(y = c(S=5000,I1=10,I2=0,C1=0,iso=0), parms = NULL, times = seq(0,T,.05), func = SI, events = list(func = NULL, root = TRUE, terminalroot = 1), rootfun=rootfun)
...
# Create a data frame for ggplot (Bar plots)
values <- c(out.circ2$j[nrow(out.circ2)], out.circ2$C1[nrow(out.circ2)])
labels <- c("Cummulative new cases", "Resources used")
...
fig = g1/g2/J.circ+
 ...
 ggsave("~/Desktop/Amy R codes/Isolation_only_with_importations.png", width = 20, height = 15, units = "cm")
