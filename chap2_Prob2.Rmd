---
title: "figures for chap2 Problem 2: community isolation only_with importation"
...
g.chap2prob2 <- ggplot(DataProb, aes(x = time, y = I1, color = factor(c))) +
  ...
  geom_line(size = 1) +
  ggtitle("Effect of transmission from travelers to the community") + ylab(expression(paste("Community prevalence, ",  I[1](t)))) + xlim(0, T) + 
  theme_classic() +
...
g.U1 <- ggplot(DataProb, aes(x = time, y = U1, color = factor(c))) +
  ...
  annotate("text", x = 900, y = 0.9*U1max, label = expression(paste(U[1][max])), hjust = -0.1, vjust = -0.5, color = "black", size = 4) +
...
chap2.prob2 <- g.chap2prob2 / g.U1 + plot_annotation(tag_levels = "A")
#& theme(plot.tag = element_text(size = 25))

chap2.prob2

ggsave("~/Desktop/Work/Students/MSc/Adu-Boahen/Thesis_2024-Updates/Amys codes/figures/chap2.prob2.png", width = 15, height = 20, units = "cm")


max(DataProb$time[which(DataProb$c==0.1)])
```