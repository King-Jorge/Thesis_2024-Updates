---
title: "figures for chap2 Problem 3: Traveller isolation only"
#output: A plot of U2_[u2max] against time for different values of u2max.
date: Aug 08, 2024
---
...
g.tr <- ggplot(DataProb, aes(x = time, y = I1, color = factor(u2max))) +
  geom_line(size = 1) +
  ggtitle("Community infections increase initially") + ylab(expression(paste("Community prevalence,", I[1](t)))) + xlim(0, 50) + 
  theme_classic() +
...
g.tr.2 <- ggplot(DataProb2, aes(x = time, y = I1, color = factor(u2max))) +
  ...
  geom_line(size = 1) +
  ggtitle("Community infections decrease initially") + ylab(expression(paste("Community prevalence,", I[1](t)))) + 
  theme_classic() +
  ylim(c(0,11))+
  annotate("text", x = 0.95*T, y = 0.75*Imin, label = "Imin", color = "black", size = 4)+
  scale_color_discrete(name = expression(paste(u[2][max]))) 
  #scale_y_continuous(breaks = seq(min(out1$I1)*0, max(out1$I1), by = 200)) +
  #scale_x_continuous(breaks = seq(min(out1$time), max(out1$time), by = 5))
...
g.U2 <- ggplot(DataProb, aes(x = time, y = U2, color = factor(u2max))) +
  ...
  annotate("text", x = 15, y = 0.9*U2max1, label = expression(paste(U[2][max])), hjust = -0.1, vjust = -0.5, color = "black", size = 4) +
...
g.U2.2 <- ggplot(DataProb2, aes(x = time, y = U2, color = factor(u2max))) +
  ...
  annotate("text", x = 0.9*T, y = 0.9*U2max, label = expression(paste(U[2][max])), hjust = -0.1, vjust = -0.5, color = "black", size = 4) +
  ggtitle(" ") + ylab(expression(paste("Total isolated travelers,", U[2](T))))  + theme_classic() +
  scale_color_discrete(name = expression(paste(u[2][max]))) 
  #scale_y_continuous(breaks = seq(min(DataProb$U2), max(DataProb$U2), by = 5)) +
  #scale_x_continuous(breaks = seq(min(out1$time), max(out1$time), by = 5))
...
```{r}
chap2.prob3 <- g.tr+g.U2+g.tr.2+g.U2.2 + plot_annotation(tag_levels = "A")
#& theme(plot.tag = element_text(size = 1))

chap2.prob3
ggsave("~/Desktop/Work/Students/MSc/Adu-Boahen/Thesis_2024-Updates/Amys codes/figures/chap2.prob3.png", width = 25, height = 20, units = "cm")

max(DataProb$time[which(DataProb$u2max==0)])
max(DataProb$time[which(DataProb$u2max==0.5)])
max(DataProb$time[which(DataProb$u2max==1)])
max(DataProb$time[which(DataProb$u2max==2)])
max(DataProb$time[which(DataProb$u2max==3)])
max(DataProb$time[which(DataProb$u2max==5)])

max(DataProb2$time[which(DataProb2$u2max==0)])
max(DataProb2$time[which(DataProb2$u2max==0.5)])
max(DataProb2$time[which(DataProb2$u2max==1)])
max(DataProb2$time[which(DataProb2$u2max==3)])
max(DataProb2$time[which(DataProb2$u2max==4.75)])
max(DataProb2$time[which(DataProb2$u2max==5)])
```